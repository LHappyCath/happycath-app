...
export default function Accueil() {
  ...
  const [stats, setStats] = useState({ membres: 0, cours: 0, chequesAttente: 0 })
  ...
  async function loadData() {
    try {
      const [{ data: cours }, { data: membres }, { data: cheques }] = await Promise.all([
        supabase.from('cours').select('*').eq('jour', aujourdJour).eq('actif', true).order('heure'),
        supabase.from('membres').select('id', { count: 'exact' }).eq('actif', true),
        supabase.from('reglements').select('id', { count: 'exact' }).eq('statut', 'en_attente'),
      ])

      // Compter inscrits par cours
      const coursAvecInscrits = await Promise.all((cours || []).map(async c => {
        ...
        const { data: hist } = await supabase.from('historique').select('id').eq('cours_id', c.id).eq('date', today).maybeSingle()
        return { ...c, nb_inscrits: count || 0, appel_fait: !!hist }
      }))

      setCoursAujourdhui(coursAvecInscrits)
      setStats({
        membres: membres?.length || 0,
        cours: cours?.length || 0,
        chequesAttente: cheques?.length || 0,
      })
    } catch (err) {
      console.error(err)
    } finally {
    ...
    <div>
      ...
      <div className="card" style={{ marginBottom: 20 }}>
        ...
        {loading ? (
          ...
          <p style={{ color: '#888', fontSize: 14 }}>Aucun cours aujourd'hui</p>
          ...
          <div key={c.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderTop: '0.5px solid rgba(0,0,0,0.07)' }}>
            ...
            <button
              onClick={() => navigate(`/cours?appel=${c.id}`)}
      ...
      <div className="stats-grid" style={{ marginBottom: 20 }}>
        <div className="stat-card">
          <div className="stat-val" style={{ color: '#FF0099' }}>{stats.membres}</div>
          <div className="stat-lbl">Membres actifs</div>
        </div>
        <div className="stat-card">
          <div className="stat-val">{stats.cours}</div>
          <div className="stat-lbl">Cours aujourd'hui</div>
        </div>
        <div className="stat-card">
          <div className="stat-val" style={{ color: stats.chequesAttente > 0 ? '#b45309' : '#1a1a1a' }}>{stats.chequesAttente}</div>
          <div className="stat-lbl">Chèques en attente</div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0,1fr))', gap: 10 }}>
        {modules.map(m => (
          <div
            ...
            style={{
              gridColumn: m.full ? 'span 2' : undefined,
              background: '#fff',
              border: '0.5px solid rgba(0,0,0,0.08)',
              borderRadius: 14,
              padding: m.full ? '14px 16px' : 16,
              cursor: 'pointer',
              display: m.full ? 'flex' : 'block',
              alignItems: m.full ? 'center' : undefined,
              gap: m.full ? 14 : undefined,
              borderTop: `3px solid ${m.color === 'pink' ? '#FF0099' : m.color === 'lime' ? '#CCFF00' : 'transparent'}`,
              backgroundImage: m.color === 'both' ? 'linear-gradient(#fff,#fff) padding-box, linear-gradient(90deg,#FF0099,#CCFF00) border-box' : undefined,
